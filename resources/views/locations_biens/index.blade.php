                        ...
                        <tr class="border-b border-gray-100 odd:bg-white even:bg-gray-50 hover:bg-green-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <!-- Supprimer -->
                                    <form action="{{ route('locations_biens.destroy', $location) }}" method="POST" class="inline" id="deleteForm{{ $location->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="w-8 h-8 flex items-center justify-center rounded border border-red-500 text-red-500 hover:bg-red-50 transition" onclick="showDeleteModal('Êtes-vous sûr de vouloir supprimer la location du bien n°{{ $location->id }} ?', document.getElementById('deleteForm{{ $location->id }}'))" title="Supprimer">
