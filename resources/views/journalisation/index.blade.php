...
<div class="w-full">
    ...
    <div class="bg-white p-5 mb-6 rounded shadow-sm border border-gray-200">
        <form action="{{ route('journalisation.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <!-- Filtre Recherche -->
            <div>
                <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Recherche</label>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Nom, email, action..." 
            ...
            <div class="flex items-end">
                <a href="{{ route('journalisation.index') }}" class="w-full px-4 py-2 bg-gray-100 text-gray-700 rounded font-medium hover:bg-gray-200 transition text-center">
                ...
                <tbody class="text-sm text-gray-700">
                    @forelse($logs as $log)
                        ...
                        <tr class="border-b border-gray-100 odd:bg-white even:bg-gray-50 hover:bg-green-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                @if($log->user)
                                    <div class="text-gray-600">{{ $log->user->email }}</div>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-8 text-center text-gray-500 bg-white">
