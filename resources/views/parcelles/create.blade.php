...
<div class="max-w-5xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-sm text-gray-400">Remplissez les informations ci-dessous pour ajouter une parcelle.</p>
        </div>
        ...
        <form action="{{ route('parcelles.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="text" id="code_parcelle" name="code_parcelle" value="{{ old('code_parcelle', 'PAR' . str_pad(rand(1, 999), 3, '0', STR_PAD_LEFT)) }}" required placeholder="Ex: PAR001"
                ...
                <div>
                    ...
                    <input type="text" id="titre_foncier" name="titre_foncier" value="{{ old('titre_foncier') }}" required placeholder="Ex: TF-ABJ-2023-001"
                           class="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-200 focus:border-[#445f47] focus:ring-2 focus:ring-[#445f47]/20 outline-none transition-all">
                    @error('titre_foncier')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="relative">
                        <select id="type_terrain" name="type_terrain" required
                            ...
                            <option value="residentiel" {{ old('type_terrain') == 'residentiel' ? 'selected' : '' }}>Résidentiel</option>
                            <option value="commercial" {{ old('type_terrain') == 'commercial' ? 'selected' : '' }}>Commercial</option>
                            <option value="agricole" {{ old('type_terrain') == 'agricole' ? 'selected' : '' }}>Agricole</option>
                            <option value="industriel" {{ old('type_terrain') == 'industriel' ? 'selected' : '' }}>Industriel</option>
                        </select>
                ...
                <div>
                    ...
                    <div class="relative">
                        <select id="statut_juridique" name="statut_juridique" required
                            ...
                            <option value="propriete" {{ old('statut_juridique') == 'propriete' ? 'selected' : '' }}>Propriété</option>
                            <option value="location" {{ old('statut_juridique') == 'location' ? 'selected' : '' }}>Location</option>
                            <option value="copropriete" {{ old('statut_juridique') == 'copropriete' ? 'selected' : '' }}>Copropriété</option>
                        </select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-gray-500">
                            <i class="fa-solid fa-chevron-down text-xs"></i>
                        </div>
                ...
                <div>
                    ...
                    <input type="number" id="surface" name="surface" value="{{ old('surface') }}" required step="0.01" min="0" placeholder="Ex: 500.50"
                           class="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-200 focus:border-[#445f47] focus:ring-2 focus:ring-[#445f47]/20 outline-none transition-all">
                    @error('surface')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" id="prix_achat" name="prix_achat" value="{{ old('prix_achat') }}" step="1000" min="0" placeholder="Ex: 5000000"
                           class="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-200 focus:border-[#445f47] focus:ring-2 focus:ring-[#445f47]/20 outline-none transition-all">
                    @error('prix_achat')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" id="date_achat" name="date_achat" value="{{ old('date_achat') }}"
                ...
                <div>
                    ...
                    <input type="number" id="prix_vente" name="prix_vente" value="{{ old('prix_vente') }}" step="1000" min="0" placeholder="Ex: 8000000"
                           class="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-200 focus:border-[#445f47] focus:ring-2 focus:ring-[#445f47]/20 outline-none transition-all">
                    @error('prix_vente')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" id="date_vente" name="date_vente" value="{{ old('date_vente') }}"
                           class="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-200 focus:border-[#445f47] focus:ring-2 focus:ring-[#445f47]/20 outline-none transition-all text-gray-600">
                    @error('date_vente')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="relative">
                        <select id="acheteur_id" name="acheteur_id"
                            ...
                            @foreach(App\Models\Proprietaire::all() as $proprietaire)
                                <option value="{{ $proprietaire->id }}" {{ old('acheteur_id') == $proprietaire->id ? 'selected' : '' }}>
                                    {{ $proprietaire->nom }} - {{ $proprietaire->email }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                    <label class="flex items-center space-x-2 cursor-pointer">
                        <input type="checkbox" name="documents_cadastraux[]" value="Titre foncier" class="rounded text-[#445f47] focus:ring-[#445f47]">
                        <span class="text-sm text-gray-700">Titre foncier</span>
                    </label>
                    <label class="flex items-center space-x-2 cursor-pointer">
                        <input type="checkbox" name="documents_cadastraux[]" value="Plan cadastral" class="rounded text-[#445f47] focus:ring-[#445f47]">
                        <span class="text-sm text-gray-700">Plan cadastral</span>
                    </label>
                    <label class="flex items-center space-x-2 cursor-pointer">
                        <input type="checkbox" name="documents_cadastraux[]" value="Certificat de propriété" class="rounded text-[#445f47] focus:ring-[#445f47]">
                        <span class="text-sm text-gray-700">Certificat de propriété</span>
                    </label>
                    <label class="flex items-center space-x-2 cursor-pointer">
                        <input type="checkbox" name="documents_cadastraux[]" value="Acte de vente" class="rounded text-[#445f47] focus:ring-[#445f47]">
                        <span class="text-sm text-gray-700">Acte de vente</span>
                    </label>
                    <label class="flex items-center space-x-2 cursor-pointer">
                        <input type="checkbox" name="documents_cadastraux[]" value="Permis de construire" class="rounded text-[#445f47] focus:ring-[#445f47]">
                        <span class="text-sm text-gray-700">Permis de construire</span>
                    </label>
                    <label class="flex items-center space-x-2 cursor-pointer">
                        <input type="checkbox" name="documents_cadastraux[]" value="Déclaration d'impôt" class="rounded text-[#445f47] focus:ring-[#445f47]">
            ...
            <div class="flex items-center justify-end gap-4 mt-8 pt-6 border-t border-gray-100">
                <a href="{{ route('parcelles.index') }}" class="px-6 py-2.5 rounded-lg text-gray-600 hover:bg-gray-100 font-medium transition-colors">
                    Annuler
                </a>
